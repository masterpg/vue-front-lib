...
import { dateTimeFormats } from '@/app/i18n/date-time-formats'
...
class AppI18n extends VueI18n {
  ...
  constructor() {
    ...
    this.supportLocales = [new LocaleData('ja-JP'), new LocaleData('en-US')]
    this.defaultLocale = this.supportLocales[0]
  }
   ...
   * 例: ['ja-JP', 'en-US', 'de-DE']
   */
  protected readonly supportLocales: LocaleData[]

  /**
   * デフォルトのロケール(言語+国)です。
   * 例: 'ja-JP'
   */
  protected readonly defaultLocale: LocaleData
  ...
  protected async importLanguage(language: string): Promise<any> {
    const msgs = await import(/* webpackChunkName: "lang-[request]" */ `@/app/i18n/lang/${language}`)
    return msgs.default
  }
