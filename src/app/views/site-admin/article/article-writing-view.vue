  ...
  export const clazz = defineComponent({
    ...
    setup(props: Readonly<Props>, ctx) {
      ...
      const load: ArticleWritingView['load'] = async articlePath => {
        ...
        // 記事ソースを取得
        await (async () => {
          const node = pageService.getStorageChildren({ path: articlePath }).find(node => node.article?.file?.type === 'Master')
          if (!node) {
        ...
        // 下書きのソースを取得
        await (async () => {
          const node = pageService.getStorageChildren({ path: articlePath }).find(node => node.article?.file?.type === 'Draft')
          if (!node) {
