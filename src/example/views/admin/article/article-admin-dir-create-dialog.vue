...
import { BaseDialog, NoCache, StorageArticleNodeType, StorageNode } from '@/lib'
...
interface OpenParams {
  parent: string
  type: StorageArticleNodeType
}

@Component
class BaseDialogMixin extends BaseDialog<OpenParams, string> {}

@Component
export default class ArticleAdminDirCreateDialog extends mixins(BaseDialogMixin, ArticleAdminPageMixin) {
  //----------------------------------------------------------------------
  //
  //  Variables
  //
  ...
  private get m_title(): string {
    if (this.params) {
      const sth = getArticleNodeTypeLabel(this.params.type)
      return String(this.$t('common.createSth', { sth }))
    } else {
      return ''
    }
  }
  ...
  private get m_icon(): string {
    if (this.params) {
      return getArticleNodeTypeIcon(this.params.type)
    } else {
      return ''
    }
  }
  ...
  private get m_parentPath(): string {
    if (!this.m_parentNode) {
      return path.join(this.pageStore.rootNode.name, '/')
    } else {
      return path.join(this.pageStore.rootNode.name, this.m_parentNode.path, '/')
    }
  }
  ...
  open(params: OpenParams): Promise<string> {
    if (params.parent === this.pageStore.rootNode.path) {
      this.m_parentNode = null
    } else {
      this.m_parentNode = this.storageLogic.sgetNode({ path: params.parent })
    }
  ...
  private m_validate(): boolean {
    // ディレクトリ名必須入力チェック
    if (this.m_dirName === '') {
      const target = String(this.$t('common.sthName', { sth: getArticleNodeTypeLabel(this.params!.type) }))
