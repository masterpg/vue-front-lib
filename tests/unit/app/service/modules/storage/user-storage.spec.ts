...
describe('AppStorageService', () => {
  ...
  beforeEach(async () => {
    provideDependency(({ service }) => {
      ...
      service.userStorage.basePath.value = basePath
      // ベースパスノードの作成
      basePathNodes = []
      ...
      // ショートハンド用変数にメソッドを設定
      toBasePath = service.userStorage.toBasePath
      toBasePaths = service.userStorage.toBasePaths
      toBasePathNode = service.userStorage.toBasePathNode
      toBasePathNodes = service.userStorage.toBasePathNodes
      toFullPath = service.userStorage.toFullPath
      toFullPaths = service.userStorage.toFullPaths
    })
  })
