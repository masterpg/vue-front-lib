...
import { StorageArticleNodeType, StorageNode, StorageNodeShareSettings, StorageNodeType } from '@/app/logic'
import { StorageState, StorageStore, StorageStoreImpl } from '@/app/logic/store'
import { cloneTestStorageNode, newTestStorageDirNode, newTestStorageFileNode } from '../../../../../../helpers/app/storage'
import { Component } from 'vue-property-decorator'
import { GENERAL_USER } from '../../../../../../helpers/app/data'
import { TestStore } from '../../../../../../helpers/app/store'
...
import { sortStorageTree } from '@/app/logic/base'
...
describe('all', () => {
  it('ベーシックケース', () => {
    ...
    storageStore.initState({
      all: sortStorageTree([d1, d11, d111, f1111, d12, f11, d2, d21, f211, f1]),
    })
...
describe('getChildren', () => {
  it('ベーシックケース', () => {
    ...
    const d2 = newTestStorageDirNode('d2')
    storageStore.initState({
      all: sortStorageTree(cloneDeep([d1, d11, f111, d12, d2])),
    })
...
describe('remove', () => {
  ...
  it(`ディレクトリ名があるディレクトリ名をベースにしている場合（'d1'と'd1_bk'のように）`, () => {
    ...
    storageStore.initState({
      all: sortStorageTree([d1, d11, d111, fileA, fileB, d11_bk, fileC]),
    })
...
describe('move', () => {
  ...
  it('存在しないパスを指定した場合', () => {
    ...
    const d2 = newTestStorageDirNode('d2')
    storageStore.initState({
      all: sortStorageTree(cloneDeep([d1, d2])),
    })
...
describe('rename', () => {
  ...
  it('ファイルのリネーム', () => {
    ...
    const f111 = newTestStorageFileNode('d1/d11/f111.txt')
    storageStore.initState({
      all: sortStorageTree(cloneDeep([d1, d11, f111])),
    })
