...
import { MarkdownItVueComponent, getMarkdownItVueComponentBodyData, getMarkdownItVueComponentPropsData } from '@/app/markdown-it'
...
class TestCompCommon extends Vue {
  @Prop({
    default: () => {
  ...
  get displayValue(): string {
    const name = `name: ${this.person.name}`
    const age = `age: ${this.person.age}`
    const arr = `arr: [${this.arr.join(', ')}]`
    const flag = `flag: ${this.flag}`
    const num = `num: ${this.num}`
...
class TestCompInline extends Vue {
...
class TestCompBlock extends Vue {
  @Prop({ default: 'Anonymous' })
  yourName!: string
}
...
describe('インライン/ブロック - ｢breaks: false｣の場合', () => {
  mount(TestCompInline)
  mount(TestCompBlock)
  const md = new MarkdownIt({ breaks: false }).use(MarkdownItVueComponent, {
    components: { TestCompInline, TestCompBlock },
  })
