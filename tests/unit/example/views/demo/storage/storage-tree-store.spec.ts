...
import { StorageTreeNode, StorageTreeStore, newStorageTreeStore } from '@/example/views/base/storage'
...
describe('pullInitialNodes', () => {
  ...
  it('ベーシックケース', async () => {
    ...
    await treeStore.pullInitialNodes(d11.path)
...
describe('pullChildren', () => {
  ...
  it('dirPathを指定した場合 - ルートノード配下のディレクトリを指定', async () => {
    ...
    await treeStore.pullChildren(d1.path)
    const actual = treeStore.getAllNodes()
