import { AuthStatus, User, UserClaims, UserInput } from '@/app/service'
import { RawEntity, toEntities } from '@/app/service/api/base'
import { pickProps, removeStartDirChars } from 'web-base-lib'
import { APIContainer } from '@/app/service/api'
...
namespace TestAPIContainer {
  ...
  export function mix<T extends APIContainerImpl>(api: T): TestAPIContainer & T {
    ...
    function _setTestAuthToken(token: string | undefined): void {
      td.replace(require('@/app/service/base'), 'getIdToken', () => token)
      td.replace(require('@/app/service/base'), 'sgetIdToken', () => token)
    }

    async function _getTestAuthToken(): Promise<string | undefined> {
      let idToken: string | undefined
      try {
        idToken = await require('@/app/service/base').getIdToken()
      } catch {}
      return idToken
    }
